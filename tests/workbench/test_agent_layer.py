...
def test_no_op_dsl_is_returned_as_actionable_tool_error() -> None:
    result = default_tool_registry().execute(
        LLMToolCall(
            call_id="no-op-rule",
            name="test_sound_law",
            arguments={"dsl": "p > p / #_", "source_child_ids": ["A"]},
        ),
        context(),
    )
    assert not result.ok
    assert result.error is not None
    assert "identity reconstruction with an empty rule set" in result.error.message


def test_no_op_rule_is_rejected_from_cascade_preview() -> None:
    result = default_tool_registry().execute(
        LLMToolCall(
            call_id="no-op-cascade",
...
def test_commit_requires_and_reuses_exact_validation() -> None:
    ...
    assert state.commit.parsed_rules[0].rule.rule_id.startswith("rule-")
    assert state.commit.parsed_rules[0].source_child_ids == ("A", "B")


def test_cascade_derives_non_linguistic_rule_label() -> None:
    result = default_tool_registry().execute(
        LLMToolCall(
            call_id="cascade-with-derived-id",
            name="test_rule_cascade",
            arguments={
                "rules": [
                    {
    ...
    assert result.result["rules"][0]["rule"]["rule_id"].startswith("rule-")
...
def test_segmentation_overlay_remaps_partial_membership_positions() -> None:
    partial_form = LexicalForm(
        form_id="A:water",
        variety_id="A",
        concept_id="water",
        segments=("p", "a"),
        cognate_memberships=(
            CognateMembership(
                ...
                interpretation=CognateMembershipInterpretation.PARTIAL_COGNATE,
                segment_indices=(1,),
                slice_unit="segment",
                provenance=CognateMembershipProvenance(
                    source_table="CognateTable",
                    source_row=2,
                    source_cognateset_id="water-1",
                    source_segment_slice=("2",),
                    source_slice_unit="segment",
                ),
            ),
        ),
    )
    ...
    result = default_tool_registry().execute(
        LLMToolCall(
            ...
            arguments={
                ...
                "rationale": "mark a boundary before the form",
            },
        ),
        state,
    )
    assert result.ok and result.result is not None
    membership = result.result["forms"][0]["cognate_memberships"][0]
    assert membership["segment_indices"] == [2]

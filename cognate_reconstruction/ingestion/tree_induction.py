...
def _lexstat_rows(lexicons: tuple[LanguageLexicon, ...]) -> dict[int, list[object]]:
    ...
    for lexicon in lexicons:
        for form in lexicon.forms:
            # LexStat accepts one whole-form cognate ID per row. Use source
            # cognacy only when it is unambiguous; partial and alternative
            # memberships are retained in the payload but are not silently
            # weighted in this exploratory fallback.

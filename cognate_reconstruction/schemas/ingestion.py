...
from cognate_reconstruction.schemas.historical import HistoricalFormBinding
...
class WorkbenchPayload(WorkbenchModel):
    ...
    historical_form_bindings: tuple[HistoricalFormBinding, ...] = ()
    ...
    def validate_varieties(self) -> WorkbenchPayload:
        ...
        sources = [
            ...
            for binding in self.historical_form_bindings
        ]
        if len(sources) != len(set(sources)):
            raise ValueError(
                "historical source varieties must be unique within a payload"
            )
        if self.historical_form_bindings and self.newick is None:
            raise ValueError(
                "historical form roles require a supplied Newick tree with "
                "explicit internal node IDs"
            )
        return self
